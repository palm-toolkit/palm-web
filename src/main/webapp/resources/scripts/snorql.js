...
function Snorql() {
    ...
    this._endpoint = baseUrl + '/sparqlview/sparql';//document.location.href.match(/^([^?]*)snorql\//)[1] + 'sparql';
    // modify these to your likeing
    this._poweredByLink = 'http://snorql.org';
    this._poweredByLabel = 'snorql';
    ...
    this.entry = function(){
        ...
        var resultTitle = 'List linked data';
    ...
    this.loadQueries=function(selector){
        ...
        var filters={}
        $.getJSON( baseUrl + "/sparqlview/sparql/queries", function(queries){
...
//            	});
    ...
    this._getLinkMaker = function(varName) {
        if (varName == 'property') {
            return function(uri) { return baseUrl + '/sparqlview?property=' + encodeURIComponent(uri); };
        } else if (varName == 'class') {
            return function(uri) { return baseUrl + '/sparqlview?class=' + encodeURIComponent(uri); };
        } else {
            return function(uri) { return baseUrl + '/sparqlview?describe=' + encodeURIComponent(uri); };
        }
    }
    ...
    this._formatURI = function(node, varName) {
        ...
        if (match) {
            ...
            img.src = baseUrl +'/resources/images/link.png';
            img.alt = '[' + match[1] + ']';
