...
<script>
	$( function(){
		...
		var options ={
			source : "<@spring.url '/venue/topicCompositionUniCloud' />",
			queryString : "",
			id: "",
			...
			onRefreshDone: function(  widgetElem , data ){
				...
				var topicModelEvolutionWidget = $.PALM.boxWidget.getByUniqueName( 'event_topicmodel_evolution' ); 
...
function visualizeTextCloud( words ){
  ...
  .fontSize(function(d) {
		...
		if( fontsize < 70 )
			fontsize = 10;
		else if( fontsize < 150 && fontsize >= 70 )
			fontsize = 12;
		else if( fontsize < 400 && fontsize >= 150 )
			fontsize = 14;
		else if( fontsize < 600 && fontsize >= 400 )
			fontsize = 16;	
		else if( fontsize < 800 && fontsize >= 600 )
			fontsize = 18;
		else if( fontsize < 1000 && fontsize >= 800 )
			fontsize = 20;
		else 
			fontsize = 22;
		return fontsize;
