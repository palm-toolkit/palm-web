...
-->
		...
		var options ={
			source : "<@spring.url '/circle/interest' />",
			queryString : "",
			id: "",
			onRefreshStart: function( widgetElem ){
				<#-- show pop up progress log -->
				$.PALM.popUpMessage.create( "Extracting circle interest", { uniqueId:uniquePidInterestCloud, popUpHeight:40, directlyRemove:false , polling:false});
						},
			onRefreshDone: function(  widgetElem , data ){
			
				<#-- check for interest evolution widget -->
				var interestEvolutionWidget = $.PALM.boxWidget.getByUniqueName( 'circle_interest_evolution' ); 
