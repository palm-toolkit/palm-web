<div class="box-body">
	<form role="form" id="manageCircleWidget" action="<@spring.url '/circle/widget' />" method="post">
		<div class="palm-table">
			<div class="table-header">
		  	...
		  	<ul id="widget-list-container" class="list-unstyled table-body" style="width:100%">
	            <#if circleWidgetsWrapper.circleWidgets?has_content>
	                <#list circleWidgetsWrapper.circleWidgets as userWidget>
			           ...
			           <li class="table-row">
			           		<input type="hidden" class="wposition" name="circleWidgets[${userWidget_index}].widget.pos" value="${userWidget_index}">
							...
							<span style="width:20%">
								<div class="btn-group">
								  <input type="hidden" name="circleWidgets[${userWidget_index}].widget.width" value="${userWidget.widget.widgetWidth}" >
							...
							<span style="width:20%">
				            	<div class="btn-group">
			                      ...
			                      <#else>
			                      	  <input type="hidden" name="circleWidgets[${userWidget_index}].widget.height" value="${userWidget.widget.widgetHeight}" >
				                      <button type="button" class="btn btn-sm btn-default min-w-75 dropdown-toggle" data-toggle="dropdown" aria-expanded="false">
							...
							<span>
				            	<div class="btn-group" data-toggle="buttons">
				            		<#if userWidget.widgetStatus == "ACTIVE">
			                          	<label class="btn btn-sm btn-default active"><input type="radio" name="circleWidgets[${userWidget_index}].widget.status" value="active" checked>On</label>
	          							<label class="btn btn-sm btn-default"><input type="radio" name="circleWidgets[${userWidget_index}].widget.status" value="nonactive">Off</label>
          							<#else>
          								<label class="btn btn-sm btn-default"><input type="radio" name="circleWidgets[${userWidget_index}].widget.status" value="active">On</label>
          								<label class="btn btn-sm btn-default active"><input type="radio" name="circleWidgets[${userWidget_index}].widget.status" value="nonactive" checked>Off</label>
          							</#if>
		                        </div>
							</span>
			           </li>
							...
							<span style="width:20%">
				            	<div class="btn-group">
			                      <#if !isHeightAvailable>
				                      <button type="button" class="btn btn-sm btn-default min-w-75 btn-flat disabled">Fixed</button>
			                      <#else>
			                      	  <input type="hidden" name="circleWidgets[${notInstalledWidget_index}].widget.height" value="${notInstalledWidget.widgetHeight}" >
				                      <button type="button" class="btn btn-sm btn-default min-w-75 dropdown-toggle" data-toggle="dropdown" aria-expanded="false">
	 ...
	 var sortableWidgetContainer = $( "#widget-list-container" ).sortable({
	  ...
	  <#-- submit via ajax -->
		...
		$( "#submit" ).click( function( e ){
			...
			$.post( $("#manageCircleWidget").attr( "action" ), $("#manageCircleWidget").serialize(), function( data ){
				<#-- todo if error -->
