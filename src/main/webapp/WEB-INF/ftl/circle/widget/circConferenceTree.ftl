...
<script>
	$( function(){
       ...
       $("#boxbody-${wUniqueName}>.tree-container").slimscroll({
		...
		var options ={
			source : "<@spring.url '/circle/academicEventTree' />",
			query: "",
			queryString : "",
			page:0,
			maxresult:50,
			...
			onRefreshDone: function(  widgetElem , data ){
							...
							if( data.count == 0 ){
								$.PALM.callout.generate( targetContainer , "warning", "Empty Conferences!", "No conferencer/journals found (insufficient data)" );
								return false;
							}
