<div id="boxbody-${wUniqueName}" class="box-body no-padding">
  	<div class="recommendedauthor-list">
    </div>
</div>
...
<script>
	$( function(){
		...
		<#-- add slim scroll -->
       $("#boxbody-${wUniqueName}>.recommendedauthor-list").slimscroll({
			height: "300px",
	        size: "6px",
		...
		var options ={
			source : "<@spring.url '/researcher/recommendedAuthorList' />",
			query: "",
			queryString : "",
			page:0,
			maxresult:10,
			...
			onRefreshDone: function(  widgetElem , data ){
							...
							var targetContainer = $( widgetElem ).find( ".recommendedauthor-list" );
							...
							if( data.count > 0 ){
								...
								$.each( data.recommendedAuthors, function( index, item){
									var researcherDiv = 
									$( '<div/>' )
										.addClass( 'author' )
										.attr({ 'id' : item.id });
